...
@pytest.mark.asyncio
async def test_current_info_gate_passes_when_other_tool_called():
    """Multi-intent turn: user asks weather + says they're considering a movie.
    Required tool for the weather domain is `research`, but the model picks
    `set_reminder` to handle the movie-plan part. The gate must NOT force
    the fallback — the model engaged with the message, just not with the
    weather half. Forcing fallback there would destroy the half it handled
    correctly. Gate only triggers when NO tool was called at all.
    """
    from src.core.task_types import RuntimeOptions
            ...
            SimpleNamespace(
                ...
                tool_calls=[
                    ToolCallRequest(
                        id="c1",
                        name="set_reminder",
                        arguments={"text": "看电影", "when": "明天 19:00"},
                    ),
                ],
                continuation_message={
                    "role": "assistant",
    ...
    result = await runtime.complete_chat(
        ...
        event_bus=None,
        runtime_options=RuntimeOptions(
            required_tool_names=("research",),
            current_info_domain="weather",
        ),
    )

    # Model's reply must NOT be replaced with the honest fallback —
    # set_reminder was a successful, intent-relevant tool call, even though
    # it doesn't intersect required={research}.
    assert "提醒已设好" in result
    assert "天气信息" not in result, (
        "fallback must not engage when at least one tool was called — "
        f"got {result!r}"
    )
